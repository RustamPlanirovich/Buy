...
public class ThisMonthViewRecycle extends DialogFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        /*Инициализируем строку с текущим месяцем взятым из Month(+1 так как месяца в Java
        нумеруются с 0)*/
        String thisMonthAmount = String.valueOf(Month+1);

        //Инициализируем db для открытия БД для чтения/записи
        db = dbHelper.getWritableDatabase();
        //Объявляем и инициализируем курсор thisMonthCursor который подгружает данные только за этот месяц
        ...
        //Закрываем курсор thisMonthCursor
